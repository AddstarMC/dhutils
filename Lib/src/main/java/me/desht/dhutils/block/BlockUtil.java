...
public class BlockUtil {
	 ...
	 *
	 * @param facing the Blockface to check
	 * @return the BlockFace 90 degrees to the left
	 */
	 ...
	 * @return Get the nearest face on the given block relative to the given location.

	 */
	@Nullable
	public static BlockFace getNearestFace(@NotNull Block block, @NotNull Location loc) {
	 ...
	 */
	public static boolean isAttachableDetached(@NotNull Block block) {
	...
	@NotNull
	public static BlockAndPosition getTargetPoint(@NotNull Player player, Set<Material> transparent, int maxDistance) {
		List<Block> lastBlocks = player.getLastTwoTargetBlocks(transparent, maxDistance);
		Block block = lastBlocks.get(1);
		BlockFace face = block.getFace(lastBlocks.get(0));
	...
	private static Vector isectLinePlane(@NotNull Vector p0, @NotNull Vector p1, @NotNull Vector plane, @NotNull Vector normal, double epsilon) {
